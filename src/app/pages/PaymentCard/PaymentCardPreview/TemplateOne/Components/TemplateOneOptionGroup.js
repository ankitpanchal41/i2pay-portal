...
const TemplateOneOptionGroup = ({ productDetails, onChangeSetProductVariant }) => {
    ...
    return (
        <>
            ...
            {productDetails?.options ? <div className="text-slate-600 mb-2 text-left text-big font-bold">Options:</div> : null}

            {productDetails?.options &&
                Object.keys(productDetails?.options).map((optionLabel, mainIndex) => (
                    <div className="grid grid-cols-3 my-1" key={mainIndex}>
                        <div className="text-big col-span-1 text-slate-600 mb-1 text-left capitalize mr-2">{optionLabel}</div>
