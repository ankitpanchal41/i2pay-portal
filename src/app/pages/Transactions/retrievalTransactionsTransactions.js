...
import { getTransactionsRetrieval } from "../../redux/actions/Transactions";
...
const RetrievalTransactions = () => {
    const dispatch = useDispatch();
    const [isOpenMenu, setIsOpenMenu] = useState(false);
    const [isLoading, setIsLoading] = useState(false);
    const [isLoadingExport, setIsLoadingExport] = useState(false);
    const { transactions, totalPage } = useSelector((state) => state.transactionRetrieval);
    ...
    useEffect(() => {
        ...
        setIsReset(false);
        dispatch(
            getTransactionsRetrieval(currentPage, perPage, searchQuery, advanceSearchPayload, () => {
                setIsLoading(false);
                setIsParPage(true);
            }),
        );
    ...
    useEffect(() => {
        if (isParPage) {
            ...
            setIsLoading(true);
            setIsReset(false);
            dispatch(getTransactionsRetrieval(1, perPage, searchQuery, advanceSearchPayload, () => setIsLoading(false)));
        }
    }, [perPage]);
    ...
    const advanceSearchSubmit = (values) => {
        ...
        setCurrentPage(1);
        dispatch(
            getTransactionsRetrieval(1, perPage, searchQuery, values, () => {
                setIsLoading(false);
                setIsParPage(true);
            }),
        );
    ...
    const resetFilter = () => {
        ...
        setCurrentPage(1);
        dispatch(
            getTransactionsRetrieval(1, perPage, "", "", () => {
                setIsLoading(false);
                setIsParPage(true);
            }),
        );
                ...
                <table class="table table-report sm:mt-2">
                    <thead>
                        <tr>
                            ...
                            <th className="lg:justify-left items-left">Retrieval Date</th>
    ...
    return (
        <>
            <AdvanceSearchModal
                ...
                resetState={isReset}
                additionalStartDateLabel={"Retrieval Start Date"}
                additionalEndDateDateLabel={"Retrieval End Date"}
                additionalStartDateName={"retrieval_start_date"}
                additionalEndDateDateName={"retrieval_end_date"}
            />
...
export default RetrievalTransactions;
