...
import { getTransactionsRemoveRetrieval } from "../../redux/actions/Transactions";
...
const RemoveRetrievalTransactions = () => {
    const dispatch = useDispatch();
    const [isOpenMenu, setIsOpenMenu] = useState(false);
    const [isLoading, setIsLoading] = useState(false);
    const [isLoadingExport, setIsLoadingExport] = useState(false);
    const { transactions, totalPage } = useSelector((state) => state.transactionRemoveRetrieval);
    ...
    useEffect(() => {
        ...
        setIsReset(false);
        dispatch(
            getTransactionsRemoveRetrieval(currentPage, perPage, searchQuery, advanceSearchPayload, () => {
                setIsLoading(false);
                setIsParPage(true);
            }),
        );
    ...
    useEffect(() => {
        if (isParPage) {
            ...
            setIsLoading(true);
            setIsReset(false);
            dispatch(getTransactionsRemoveRetrieval(1, perPage, searchQuery, advanceSearchPayload, () => setIsLoading(false)));
        }
    }, [perPage]);
    ...
    const advanceSearchSubmit = (values) => {
        ...
        setCurrentPage(1);
        dispatch(
            getTransactionsRemoveRetrieval(1, perPage, searchQuery, values, () => {
                setIsLoading(false);
                setIsParPage(true);
            }),
        );
    ...
    const resetFilter = () => {
        ...
        setCurrentPage(1);
        dispatch(
            getTransactionsRemoveRetrieval(1, perPage, "", "", () => {
                setIsLoading(false);
                setIsParPage(true);
            }),
        );
    ...
    const onClickExport = async () => {
        ...
        const payload = {
            ...
            search: searchQuery,
            test_mode: 0,
            is_remove_retrieval: 1,
            txn_status: "remove_retrieval",
        };
    ...
    const _renderHeading = () => {
        return (
            <Heading
                title={"Remove Retrieval Transactions"}
                ...
                <table class="table table-report sm:mt-2">
                    <thead>
                        <tr>
                            ...
                            <th className="lg:justify-left items-left">Remove Retrieval Date</th>
                            ...
                            {transactions?.map((item, index) => {
                                return (
                                    <tr className="intro-x" key={index}>
                                        ...
                                        <td>{item.remove_retrieval_date}</td>
                                            ...
                                            <div className="flex lg:ml-0 lg:justify-end mt-3 lg:mt-0">
                                                <div className="cursor-pointer flex text-slate-500 text-xs">
                                                    <div>
                                                        ...
                                                        <div className="flex text-slate-500 text-xs">
                                                            <span className="text-slate-900 dark:text-white text-xs mt-0.5 font-bold">
                                                                Remove Retrieval Date :&nbsp;
                                                            </span>
                                                            <span className="text-slate-900 dark:text-white text-xs mt-0.5">
                                                                {item?.remove_retrieval_date}
                                                            </span>
                                                        </div>
                                                        <div className="flex text-slate-500 text-xs">
                                                            <span className="text-slate-900 dark:text-white text-xs mt-0.5 font-bold">
    ...
    return (
        <>
            <AdvanceSearchModal
                ...
                resetState={isReset}
                additionalStartDateLabel={"Remove Retrieval Start Date"}
                additionalEndDateDateLabel={"Remove Retrieval End Date"}
                additionalStartDateName={"remove_retrieval_start_date"}
                additionalEndDateDateName={"remove_retrieval_end_date"}
            />
...
export default RemoveRetrievalTransactions;
