...
import { getTransactionsSuspicious } from "../../redux/actions/Transactions";
...
const SuspiciousTransactions = () => {
    ...
    const { transactions, totalPage } = useSelector((state) => state.transactionsSuspicious);
    ...
    useEffect(() => {
        ...
        setIsReset(false);
        dispatch(
            getTransactionsSuspicious(currentPage, perPage, searchQuery, advanceSearchPayload, () => {
                setIsLoading(false);
                setIsParPage(true);
            }),
        );
    ...
    useEffect(() => {
        if (isParPage) {
            ...
            setIsLoading(true);
            setIsReset(false);
            dispatch(getTransactionsSuspicious(1, perPage, searchQuery, advanceSearchPayload, () => setIsLoading(false)));
        }
    }, [perPage]);
    ...
    const advanceSearchSubmit = (values) => {
        ...
        setCurrentPage(1);
        dispatch(
            getTransactionsSuspicious(1, perPage, searchQuery, values, () => {
                setIsLoading(false);
                setIsParPage(true);
            }),
        );
    ...
    const resetFilter = () => {
        ...
        setCurrentPage(1);
        dispatch(
            getTransactionsSuspicious(1, perPage, "", "", () => {
                setIsLoading(false);
                setIsParPage(true);
            }),
        );
                ...
                <table class="table table-report sm:mt-2">
                    <thead>
                        <tr>
                            ...
                            <th className="lg:justify-left items-left">Suspicious Date</th>
    ...
    return (
        <>
            <AdvanceSearchModal
                ...
                resetState={isReset}
                additionalStartDateLabel={"Suspicious Start Date"}
                additionalEndDateDateLabel={"Suspicious End Date"}
                additionalStartDateName={"suspicious_start_date"}
                additionalEndDateDateName={"suspicious_end_date"}
            />
