...
import { getTransactionsRemoveSuspicious } from "../../redux/actions/Transactions";
...
const RemoveSuspiciousTransactions = () => {
    const dispatch = useDispatch();
    const [isOpenMenu, setIsOpenMenu] = useState(false);
    const [isLoading, setIsLoading] = useState(false);
    const [isLoadingExport, setIsLoadingExport] = useState(false);
    const { transactions, totalPage } = useSelector((state) => state.transactionsRemoveSuspicious);
    ...
    useEffect(() => {
        ...
        setIsReset(false);
        dispatch(
            getTransactionsRemoveSuspicious(currentPage, perPage, searchQuery, advanceSearchPayload, () => {
                setIsLoading(false);
                setIsParPage(true);
            }),
        );
    ...
    useEffect(() => {
        if (isParPage) {
            ...
            setIsLoading(true);
            setIsReset(false);
            dispatch(getTransactionsRemoveSuspicious(1, perPage, searchQuery, advanceSearchPayload, () => setIsLoading(false)));
        }
    }, [perPage]);
    ...
    const advanceSearchSubmit = (values) => {
        ...
        setCurrentPage(1);
        dispatch(
            getTransactionsRemoveSuspicious(1, perPage, searchQuery, values, () => {
                setIsLoading(false);
                setIsParPage(true);
            }),
        );
    ...
    const resetFilter = () => {
        ...
        setCurrentPage(1);
        dispatch(
            getTransactionsRemoveSuspicious(1, perPage, "", "", () => {
                setIsLoading(false);
                setIsParPage(true);
            }),
        );
    ...
    const onClickExport = async () => {
        ...
        const payload = {
            ...
            search: searchQuery,
            test_mode: 0,
            is_remove_suspicious: 1,
            txn_status: "remove_suspicious",
        };
    ...
    const _renderHeading = () => {
        return (
            <Heading
                title={"Remove Suspicious Transactions"}
                ...
                <table class="table table-report sm:mt-2">
                    <thead>
                        <tr>
                            ...
                            <th className="lg:justify-left items-left">Remove Suspicious Date</th>
                            ...
                            {transactions?.map((item, index) => {
                                return (
                                    <tr className="intro-x" key={index}>
                                        ...
                                        <td>{item.remove_suspicious_date}</td>
                                            ...
                                            <div className="flex lg:ml-0 lg:justify-end mt-3 lg:mt-0">
                                                <div className="cursor-pointer flex text-slate-500 text-xs">
                                                    <div>
                                                        ...
                                                        <div className="flex text-slate-500 text-xs">
                                                            <span className="text-slate-900 dark:text-white text-xs mt-0.5 font-bold">
                                                                Remove Suspicious Date :&nbsp;
                                                            </span>
                                                            <span className="text-slate-900 dark:text-white text-xs mt-0.5">
                                                                {item?.remove_suspicious_date}
                                                            </span>
                                                        </div>
                                                        <div className="flex text-slate-500 text-xs">
                                                            <span className="text-slate-900 dark:text-white text-xs mt-0.5 font-bold">
    ...
    return (
        <>
            <AdvanceSearchModal
                ...
                resetState={isReset}
                additionalStartDateLabel={"Remove Suspicious Start Date"}
                additionalEndDateDateLabel={"Remove Suspicious End Date"}
                additionalStartDateName={"remove_suspicious_start_date"}
                additionalEndDateDateName={"remove_suspicious_end_date"}
            />
...
export default RemoveSuspiciousTransactions;
